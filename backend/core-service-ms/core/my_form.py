...
class MyForm(forms.Form):
    ...
    def __init__(self, data=None, files=None, auto_id='id_%s', prefix=None,
        ...
        super(MyForm, self).__init__(data=data, files=files, auto_id=auto_id, prefix=prefix, initial=initial,
        ...
        for field in self.fields:
            ...
            if isinstance(self.fields[field], forms.BooleanField):
                ...
                if field in self.data:
                    if str(self.data[field]).lower() in ('false', '0'):
                        value = False
                    else:
                        value = bool(self.data[field])
                fields.update({field: value})
            else:
                fields.update({field:  self.data[field] if field in self.data and self.data[field] else ''})
    ...
    def required_field(self, name_field, required_value=True):
        self.fields[name_field].required = required_value
    ...
    def clean(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            if field_name.endswith('_id') and field.required:
                value = cleaned_data.get(field_name)
                if value is None or value <= 0:
                    error_message = field.error_messages.get('required', 'Campo no debe estar vacío')
                    self.add_error(field_name, error_message)
                    self.addError(field_name, error_message)
    ...
    def to_dict(self):
        """
        Returns a dictionary of the cleaned data.
        """
        if not self.is_valid():
            self.collect_errors()
